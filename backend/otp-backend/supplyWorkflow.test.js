...
for (const mode of ['Manual', 'Online']) test(`${mode}: request to funded PO, partial receiving, matching and payment`, async () => {
  ...
  await f.act('procurement', rfq, 'close'); await f.act('procurement', quote, 'select', { justification: 'Meets specification and delivery date', category: 'Materials' })
  ...
  f.evidence(invoice); await f.act('finance', invoice, 'verify'); assert.equal(f.read(invoice).status, 'Disputed')
  ...
  await f.act('finance', invoice, 'verify'); assert.equal(f.read(invoice).status, 'Matched')
  await f.act('finance2', invoice, 'approve')
  await f.act('finance', invoice, 'pay', { proofId: f.evidence(invoice), method: 'Manual/External', reference: 'BANK1', paymentDate: date })
