...
export const reportDefinitions = {
  ...
  supplier: [named('orders', 'My Purchase Orders', orders), named('invoices', 'My Invoices', d => kind(d, 'invoice')), named('payments', 'My Payments', d => kind(d, 'payment'))],
}
...
export function dashboardCharts(data, department) {
  ...
  return [grouped('Procurement request status', kind(data, 'procurement'), 'status'), grouped('Funding request status', kind(data, 'budgetRequest'), 'status'), grouped('Purchase order status', orders(data), 'status'), grouped('Supplier confirmation status', kind(data, 'supplierConfirmation'), 'status')]
}
