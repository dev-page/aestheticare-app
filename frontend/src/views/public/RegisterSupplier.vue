...
const taxRegistrationPreviewUrl = ref('')
const businessRegistrationPreviewUrl = ref('')
const supplierDocumentUploadState = ref({
  taxRegistration: { uploading: false, progress: 0, error: '' },
  businessRegistration: { uploading: false, progress: 0, error: '' },
})
...
const registrationPhase = ref('form')
...
const selectedBusinessAddressLabel = computed(() => addressCity.value || 'Select Business Address')
...
const clearFormFields = () => {
  ...
  if (taxRegistrationPreviewUrl.value?.startsWith('blob:')) URL.revokeObjectURL(taxRegistrationPreviewUrl.value)
  if (businessRegistrationPreviewUrl.value?.startsWith('blob:')) URL.revokeObjectURL(businessRegistrationPreviewUrl.value)
  taxRegistrationPreviewUrl.value = ''
  businessRegistrationPreviewUrl.value = ''
  supplierDocumentUploadState.value = {
    taxRegistration: { uploading: false, progress: 0, error: '' },
    businessRegistration: { uploading: false, progress: 0, error: '' },
  }
  locationError.value = ''
...
const checkSupplierRegistrationStatus = async (emailValue) => {
  try {
    const res = await axios.post(`${OTP_API_BASE}/auth/check-supplier-registration-status`, {
      email: String(emailValue || '').trim().toLowerCase(),
    })
    return res?.data || null
  } catch (error) {
    console.error('Failed to check supplier registration status:', error)
    return null
  }
}

const requestSupplierOtp = async (emailValue, uidValue = '') => {
  try {
    const res = await axios.post(`${OTP_API_BASE}/auth/request-supplier-otp`, {
      email: String(emailValue || '').trim().toLowerCase(),
...
const handleSupplierDocumentChange = (documentKey, event) => {
  ...
  if (documentKey === 'taxRegistration') {
    ...
    if (taxRegistrationPreviewUrl.value?.startsWith('blob:')) URL.revokeObjectURL(taxRegistrationPreviewUrl.value)
    taxRegistrationPreviewUrl.value = selectedFile && selectedFile.type?.startsWith('image/')
      ? URL.createObjectURL(selectedFile)
      : ''
  }

  if (documentKey === 'businessRegistration') {
    businessRegistrationFile.value = selectedFile
    if (businessRegistrationPreviewUrl.value?.startsWith('blob:')) URL.revokeObjectURL(businessRegistrationPreviewUrl.value)
    businessRegistrationPreviewUrl.value = selectedFile && selectedFile.type?.startsWith('image/')
      ? URL.createObjectURL(selectedFile)
      : ''
  }
}

const uploadSupplierDocuments = async (uid) => {
  const uploadedDocuments = {}
  const entries = [
    ['taxRegistration', taxRegistrationFile.value],
    ['businessRegistration', businessRegistrationFile.value],
  ]

  for (const [documentKey, file] of entries) {
    if (!file) {
      throw new Error(`Please upload your ${documentKey === 'taxRegistration' ? 'tax registration' : 'business registration'} document.`)
    }

    supplierDocumentUploadState.value[documentKey] = { uploading: true, progress: 0, error: '' }
    try {
      const uploaded = await uploadSupplierDocument(uid, file, documentKey, (progress) => {
        supplierDocumentUploadState.value[documentKey] = {
          ...supplierDocumentUploadState.value[documentKey],
          uploading: true,
          progress,
          error: '',
      ...
      uploadedDocuments[documentKey] = uploaded
      supplierDocumentUploadState.value[documentKey] = { uploading: false, progress: 100, error: '' }
    } catch (error) {
      const message = error?.message || 'Failed to upload document.'
      supplierDocumentUploadState.value[documentKey] = { uploading: false, progress: 0, error: message }
      throw error
    }
  }
...
const uploadSupplierDocument = async (uid, file, documentKey, onProgress = () => {}) => {
  if (!file) return null
  const safeName = `${Date.now()}-${String(file.name || 'document').replace(/\s+/g, '_')}`
  const filePath = `supplier-registration/${uid}/${documentKey}/${safeName}`
...
const resendOtp = async () => {
  ...
  const otpResult = await requestSupplierOtp(otpRecipientEmail.value, userUid.value)
...
const register = async () => {
  ...
  if (!firstName.value || !lastName.value || !businessName.value || !email.value || !password.value || !confirmPassword.value) {
  ...
  if (!taxRegistrationFile.value || !businessRegistrationFile.value) {
    toast.error('Please upload both the tax registration and business registration documents.')
    return
  }
  ...
  try {
    ...
    await setDoc(doc(db, 'supplierApplications', uid), {
      ownerId: uid,
      ...
      businessAddress: String(address.value || '').trim(),
      businessAddressBuildingNumber: addressBuildingNumberEnabled.value ? addressBuildingNumber.value.trim() : '',
      businessAddressStreetName: addressStreetNameEnabled.value ? addressStreetName.value.trim() : '',
      businessAddressStreet: String(addressStreet.value || '').trim(),
      businessAddressBarangay: String(addressBarangay.value || '').trim(),
      businessAddressCity: String(addressCity.value || '').trim(),
      businessAddressProvince: String(addressProvince.value || '').trim(),
    ...
    const uploadedDocuments = await uploadSupplierDocuments(uid)
    await updateDoc(doc(db, 'supplierApplications', uid), {
      documents: uploadedDocuments,
      documentsUpdatedAt: serverTimestamp(),
    })

    otpRecipientEmail.value = normalizedEmail
    const otpResult = await requestSupplierOtp(otpRecipientEmail.value, uid)
    applyOtpRequestResult(
      otpResult,
      'OTP sent to your email. Please verify to continue.',
      'Unable to send OTP right now. You can retry using Resend OTP.'
    )
    registrationPhase.value = 'otp'
  } catch (err) {
    console.error(err)
    ...
    if (errorCode === 'auth/email-already-in-use') {
      const normalizedEmail = email.value.trim().toLowerCase()
      const statusResult = await checkSupplierRegistrationStatus(normalizedEmail)

      if (statusResult?.exists) {
        const role = String(statusResult.role || '').trim().toLowerCase()
        const status = String(statusResult.status || '').trim().toLowerCase()
        const isActiveSupplier = role === 'supplier' && (Boolean(statusResult.emailVerified) || status === 'active')
        ...
        if (isActiveSupplier) {
          toast.info('This supplier account is already verified. Please sign in instead.')
          return
        }

        if (role === 'supplier' || statusResult.canResumeOtp) {
          userUid.value = String(statusResult.uid || '').trim()
          otpRecipientEmail.value = normalizedEmail
          const otpResult = await requestSupplierOtp(otpRecipientEmail.value, userUid.value)
          applyOtpRequestResult(
            otpResult,
            'We found your supplier account. Please verify the OTP to continue.',
...
const verifyOtp = async () => {
  ...
  try {
    ...
    const verifyRes = await axios.post(`${OTP_API_BASE}/auth/verify-supplier-otp`, {
      uid: userUid.value,
      email: lookupEmail,
      otp: otpCode.value,
    })
    ...
    <div class="relative z-10 flex items-center justify-center px-4 pt-12 pb-10 text-sm">
      <div class="relative w-full max-w-6xl grid grid-cols-1 lg:grid-cols-[60%_40%] rounded-3xl overflow-hidden bg-white/68 backdrop-blur-xl border border-gold-200/60 shadow-2xl shadow-gold-900/15">
        <div class="register-form-panel relative z-10 flex items-center justify-center px-4 pt-10 pb-10 sm:px-8">
          ...
          <div v-if="registrationPhase !== 'waiting'" class="w-full">
            <form class="space-y-4 w-full max-w-[560px]" @submit.prevent="register">
              <div class="mb-2">
                ...
                <h1 class="register-title text-3xl sm:text-4xl leading-tight">Supplier Registration</h1>
                <p class="text-charcoal-600 text-sm mt-1">Supplier information, documents, and OTP verification in one secure flow.</p>
              </div>
              ...
              <div class="relative">
                <input v-model="businessName" placeholder=" " required class="peer input h-16 pt-4 pb-2 px-3" />
              ...
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="relative">
                  <label for="supplier-address" class="floating-label floating-label-raised">Business Address</label>
                  <button
                    id="supplier-address"
                    type="button"
              ...
              <div class="space-y-3 rounded-2xl border border-gold-200/80 bg-cream-50/80 p-4">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.14em] text-charcoal-600">Documents</p>
                    <p class="text-xs text-charcoal-500">Upload clear copies of your tax registration and business registration documents.</p>
                  </div>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  <div class="rounded-xl border border-gold-200/80 bg-white/80 p-3 space-y-3">
                    ...
                    <div class="rounded-lg border border-dashed border-gold-200 bg-cream-50/80 p-3">
                      ...
                      <p v-if="supplierDocumentUploadState.taxRegistration.error" class="mt-1 text-xs text-red-600">{{ supplierDocumentUploadState.taxRegistration.error }}</p>
                    </div>
                  </div>
                  ...
                  <div class="rounded-xl border border-gold-200/80 bg-white/80 p-3 space-y-3">
                    ...
                    <div class="rounded-lg border border-dashed border-gold-200 bg-cream-50/80 p-3">
                      ...
                      <p v-if="supplierDocumentUploadState.businessRegistration.error" class="mt-1 text-xs text-red-600">{{ supplierDocumentUploadState.businessRegistration.error }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <button type="submit" :disabled="isSubmitting" class="w-full py-3 rounded-xl bg-gold-700 text-white font-semibold text-base hover:bg-gold-800 hover:scale-[1.02] active:scale-[0.98] transition disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100">
                {{ isSubmitting ? 'Submitting...' : 'Continue to OTP' }}
              </button>
          ...
          <div v-else class="w-full max-w-[560px] rounded-3xl border border-gold-200/80 bg-white/80 px-6 py-10 text-center shadow-xl shadow-gold-900/10">
            <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gold-100 text-gold-700">
              <Icon icon="mdi:clipboard-check-outline" class="h-8 w-8" />
            </div>
            <h2 class="register-title text-3xl sm:text-4xl">Waiting for Approval</h2>
            <p class="mt-3 text-sm text-charcoal-600">
              Your supplier registration has been submitted successfully. We have verified your email and your application is now pending review.
            </p>
            <p class="mt-2 text-xs text-charcoal-500">
              You can safely close this page and wait for admin approval.
            </p>
            <button
              type="button"
              class="mt-6 inline-flex items-center justify-center rounded-xl bg-gold-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-gold-800"
              @click="goToRegisterChooser"
            >
      ...
      <Modal panelClass="bg-white border border-gold-200/80 w-[92vw] max-w-[34rem]" :isOpen="otpSent" :title="'Verify OTP'" @close="closeOtpModal" :showConfirm="false">
        <div class="space-y-5 p-1">
          <p class="text-sm text-charcoal-600">
            We've sent a One-Time Password (OTP) to your email. Please enter it below to verify your supplier account.
          </p>
...
.upload-input {
  ...
  border: 1px dashed rgba(214, 186, 152, 0.95);
  background: rgba(255, 255, 255, 0.72);
  padding: 0.8rem;
  color: #6f3f2a;
}
...
.upload-preview-image {
  ...
  border: 1px solid rgba(214, 186, 152, 0.4);
}
